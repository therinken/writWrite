...
function joinFirepadForHash() {
  ...
  var firepadRef = new Firebase('https://writwrite.firebaseio.com/web/uauth').child(id);
  ...
  firepad = Firepad.fromCodeMirror(firepadRef, codeMirror,
      { richTextToolbar: true, richTextShortcuts: true, userId: authData.uid});
...
function ensurePadInList(id) {
  ...
  var list = JSON.parse(localStorage.getItem('pad-list') || "{ }");
  if (!(id in list)) {
    var now = new Date();
    ...
    localStorage.setItem('pad-list', JSON.stringify(list));
    buildPadList();
  }
}
